...
| 2026-05-27 | Auswerten placeholder | `workspace-evaluate-view.tsx`: `/workspace?view=auswerten` ohne Inhalt (ersetzt durch 2026-06-04) |
| 2026-06-04 | Workspace Auswerten screen | `workspace-evaluate-view.tsx` + `workspace-evaluate-charts.tsx`: KPIs, Status/Monats-Charts, Primary-Filterleiste (`h-9`), Fakultäts-Status-Stapel; Mock-SSOT `lib/config/workspace-evaluate-mock.json`; Layout Header + `stone-50` cards `gap-6` / `p-6` (wie Terminplaner/Home) |
