...
- Berechtigungen: **Postgres RLS** und **Trigger** — Frontend ersetzt das nicht. Wichtig:
  ...
  - **R4 Workspace-Lesen:** additive Policies `applications_select_r4_workspace` und `users_select_r4_workspace_applicants` (Migration `supabase/migrations/20260513190000_r4_workspace_select_policies.sql`). **R4 Workspace-Schreiben:** Policy **`applications_update_r4_decision`** (`20260514120000_*`, erweitert **`20260603140000_r4_decision_allow_rejected_status.sql`**) — `USING` nur `in_implementation`, `WITH CHECK` `in_implementation`, `approved` oder **`rejected`**. **Pflicht:** Bedingung über **`current_user_role()`** (`SECURITY DEFINER`, bereits im Schema) — **niemals** `EXISTS (SELECT … FROM public.users …)` innerhalb von RLS auf `users`/`applications`, sonst Postgres **„infinite recursion detected in policy for relation users“**.
...
| `components/domain/r4-decision-review-blocks.tsx` | R4 Schalter-Zeilen, Fachstelle-bestätigt, Footer, Freitextzeilen «Hinzufügen/Hinzugefügt» (R1-ähnliche Eingabemechanik) |
