...
|---|---|---|
| R1 — Studierende:r | `/portal` | Antrag stellen, nach R2-Feedback **Block-Anpassungen** (`PortalApplicationAdjustment`), Freigabe zurück an Fachstelle; bei **`approved`/`rejected`** ausgestellte **Verfügung** (`ApplicationIssuedVerfuegungView`) inkl. Sidebar **«Zugehörige Dokumente»** wie R2/R4 |
...
- **Auswerten:** `/workspace?view=auswerten` → `WorkspaceEvaluateView` — **institutionelle NTA-Statistik** für die zentrale Fachstelle (R2-Zielbild); Sidebar für R2–R6; **Mock-only** (`lib/config/workspace-evaluate-mock.json` → `getEvaluateSnapshot`). Layout wie Home/Terminplaner: Header + Primary-Filterleiste + `stone-50`-Karten (`gap-6`). Inhalte: KPIs (Volumen, Pipeline, Bewilligungsquote, Median), Status-Chart (7 kanonische States), Monats-Peaks, Gültigkeit/Geltung, Massnahmen-Rankings, Prozessdauer, **Status je Fakultät** (gestapelt), Tabelle Fakultät (inkl. Pipeline/Abgelehnt), BA/MA, UZH-Kontext. Filter: Zeitraum 6/12/24 Mon., Bildungsstufe, Fakultät — skaliert nur Mock-Slices. Fakultäten mit **unterschiedlichen** `statusCounts` (sichtbare Spread bei Bewilligungsquote). Details → `Dashboard_Core_Layout_Kontext.md` § 5c
